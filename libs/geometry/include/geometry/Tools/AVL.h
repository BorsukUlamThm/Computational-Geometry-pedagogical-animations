...
namespace geometry
{
	...
	template<typename T>
	AVL_node<T>::AVL_node(const T& val, AVL_node* left_node,
						  AVL_node* right_node)
	{
		root = T(val);
		left = left_node;
	...
	protected:
		...
		Node* aux_find_left_neighbour(const T& val, Node* node,
									  Node* candidate) const;
		Node* aux_find_right_neighbour(const T& val, Node* node,
									   Node* candidate) const;
	...
	template<typename T, typename Comparator>
	void
	AVL_tree<T, Comparator>::aux_remove(const T& val, Node*& node, bool& found)
	{
		if (node == nullptr)
		{
			found = false;
