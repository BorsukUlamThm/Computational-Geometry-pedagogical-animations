...
public:
    ...
    void add_point_acquisition(const sf::Color& col = DEFAULT_PLOT_COLOR,
                               float rad = 3);
    void add_segment_acquisition(const sf::Color& line_col = DEFAULT_PLOT_COLOR,
...
void Acquisition_canvas::add_point_acquisition(const sf::Color& col, float rad)
{
    Point_acq point(col, rad);
    buffer.emplace_back(point);
}

void Acquisition_canvas::add_segment_acquisition(const sf::Color& line_col,
...
void Acquisition_canvas::setup_bounding_box()
{
    ...
    {
        gr::Point_plt p(0, 0);
        gr::Point_plt q((Coordinate(width)), Coordinate(height));
...
{
    float ratio_x = mouse_x / float(width);
    float ratio_y = mouse_y / float(height);
